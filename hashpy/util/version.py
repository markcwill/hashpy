...
NSL_ROOT = os.path.abspath(os.path.join(script_dir, os.pardir,
                                          os.pardir))
VERSION_FILE = os.path.join(NSL_ROOT, "hashpy", "RELEASE-VERSION")


def call_git_describe(abbrev=4):
    try:
        p = Popen(['git', 'rev-parse', '--show-toplevel'],
                  cwd=NSL_ROOT, stdout=PIPE, stderr=PIPE)
        p.stderr.close()
        path = p.stdout.readlines()[0].strip()
    except:
        return None
    if os.path.normpath(path) != NSL_ROOT:
        return None
    try:
