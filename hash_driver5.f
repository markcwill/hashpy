...
c The polarity input format is the "phase" format of the SCEDC, similar to FPFIT.
c Takeoff angle uncertainty is expresses as a suite of 1D velocity models.
c
...
c     13-17      f5.2   origin time, seconds
c     18-19      i2     latitude, degrees
c     20         a1     latitude, 'S'=south
c     21-25      f5.2   latitude, minutes
c     26-28      i3     longitude, degrees
c     29         a1     longitude, 'E'=east
c     30-34      f5.2   longitude, minutes
c     35-39      f5.2   depth, km
c     89-93      f5.2   horizontal location uncertainty, km
c     95-99      f5.2   vertical location uncertainty, km
c     140-143    f4.2   magnitude
c     150-165    a16    event ID
c
c polarity line:
c     columns  format   value
c     -------------------------
...
c variables for storing earthquake input information  
      character*16 icusp,icusptest  ! event ID
...
c variables for polarity information, input to HASH subroutines
      character*4 sname(npick0),snamein                ! station name
      character*3 scomp(npick0)                        ! station component
      character*2 snet(npick0)                         ! network code
      character*1 pickpol(npick0),pickpolin,pickonset(npick0)    ! polarity pick : U, u, +, D, d, or - ; onset, I or E
      integer p_pol(npick0),p_qual(npick0),spol,p_qualin        ! polarity pick (+/-1),  quality (0/1), and reversal (+/-1)
...
c file names
      ...
      print *,'Enter name of take-off angle input file (SIMULPS format)'       
      read (*,'(a)') fpfile
      open (12,file=fpfile,status='old')

      print *,'Enter name of polarity input file (FPFIT-like format)'       
      read (*,'(a)') fpfile
...
20    continue
c read in earthquake location, etc      ! Relocated earthquakes, SIMULPS-like format
      read (12,5,end=999) 
      read (12,5,end=999)
5     format()
      read (12,25,end=999) iyr,imon,idy,ihr,imn,qsec,ilatd,cns,qlatm,
     &                ilond,cew,qlonm,qdep,qmag,np,icusp,rms
25    format (1x,3i2,1x,2i2,1x,f5.2,1x,i2,a1,f5.2,1x,i3,a1,f5.2,1x,
     &              f6.2,3x,f4.2,i3,1x,a8,f5.2)
      ...
      seh=0.   ! set parameters not given in input file
      sez=0.   ! set parameters not given in input file
      terr=-9               
      nppick=-9
      nspick=-9
      evtype='L'
...
c read in stations and take-off angles       ! from SIMULPS
      read (12,5,end=999) 
      read (12,5,end=999) 
      k=1
30    continue
        read (12,35,end=40) sname(k),range,iaz,ith,sazi,sthe
35      format (1x,a4,1x,f5.1,i4,i4,46x,f4.0,f4.0)
        if (sname(k).eq.'    ')  goto 40 ! end of data for this event
        if (range.gt.delmax) goto 30
        if (sazi.eq.0.) sazi=0.5
        if (sthe.eq.0.) sthe=0.5
        qazi=real(iaz)
        qthe=180.-real(ith)
        p_qual(k)=-9
        p_azi_mc(k,1)=qazi    
...
c read in polarities              ! FPFIT-like format
      open (14,file=fpfile,status='old')
120   continue
        read (14,125,end=190) qmag2,icusptest
125     format (139x,f4.2,14x,a8)
        if (icusptest.eq.icusp) then
          iuse=1
        else
          iuse=0
        end if
130     continue
          read (14,135) snamein,pickpolin,p_qualin
135       format (a4,11x,a1,i1)
          if (snamein.eq.'    ')  goto 140 ! end of data for this event
          if (iuse==1) then
            if (qmag2.ne.0) then
              qmag=qmag2
            end if
            do k=1,npol
              if (snamein(1:3).eq.sname(k)(1:3)) goto 137
            end do
            goto 130
137         if (p_qualin.gt.1) goto 130
            if (pickpolin.eq.'U'.or.
     &                    pickpolin.eq.'u'.or.
     &                    pickpolin.eq.'+') then
              p_pol(k)=1
            else if (pickpolin.eq.'D'.or.
     &                    pickpolin.eq.'d'.or.
     &                    pickpolin.eq.'-') then
              p_pol(k)=-1
            else
            ...
            call CHECK_POL(plfile,snamein,iyr,imon,idy,ihr,spol)
...
c adjust for any missing polarities
      npol2=npol
      do k=npol,1,-1
        if (p_qual(k).eq.-9) then
          npol2=npol2-1
          do j=k,npol2
            do i=1,nmc
              p_azi_mc(j,i)=p_azi_mc(j+1,i)  
              p_the_mc(j,i)=p_the_mc(j+1,i)
            end do
            p_pol(j)=p_pol(j+1)
            p_qual(j)=p_qual(j+1)
            sname(j)=sname(j+1)
